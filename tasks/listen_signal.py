...
# 处理start信号
def start_machine(order_id, wash_system):
    wash_system.order_id = order_id
    if order_id:
        order = OrderRecord.objects.get_or_create(order_id=order_id)[0]
    logger.info("得到洗车机订单号:", str(order_id))
    print("监听到start洗车机")
    wash_system.httpcmd(0) # TODO: 重复发送 start stop reset 的情况, 待验证
